...
/*
    ScrollReveal.js v4.0.9
    https://scrollrevealjs.org/
    (c) 2019 ScrollReveal.js by @jlmakes
...
const DEFAULT_CONFIG = {  
  ...
  function getTransformMatrix(element) {  
	...
	const matrix = style.transform.match(/matrix(3d)?\(([^)]+)\)/);  
	if (matrix) {  
     return matrix[2].split(', ').map(parseFloat);  
	}  
  ...
  function getTransitionTimingFunction(element) {  
	const style = getComputedStyle(element);  
  ...
  function createTransition(element, options) {  
	const transition = {  
     property: getTransitionProperty(element),  
     duration: getTransitionDuration(element),  
     timingFunction: getTransitionTimingFunction(element),  
     delay: getTransitionDelay(element)  
	};  
	if (options.delay) {  
   ...
     transition.timingFunction = options.timingFunction;  
	}  
	return transition;  
  }  
	
  function createTransform(element, options) {  
	const transform = {  
     matrix: getTransformMatrix(element)  
	};  
	if (options.rotate) {  
     transform.matrix = transform.matrix.map(function(value, index) {  
      if (index === 0 || index === 5) {  
        return value * Math.cos(options.rotate.x);  
      }  
      if (index === 1 || index === 4) {  
        return value * Math.sin(options.rotate.x);  
      }  
      return value;  
     });  
	}  
  ...
  function createOpacity(element, options) {  
  ...
  function createInlineStyles(element, options) {  
	const styles = {  
     opacity: createOpacity(element, options),  
     transform: createTransform(element, options),  
     transition: createTransition(element, options)  
	};  
	return styles;  
  }  
	
  function revealElement(element, options) {  
	const styles = createInlineStyles(element, options);  
	element.style.opacity = styles.opacity;  
	element.style.transform = styles.transform.matrix.join(', ');  
	element.style.transition = styles.transition.property + ' ' + styles.transition.duration + ' ' + styles.transition.timingFunction + ' ' + styles.transition.delay;  
  }  
